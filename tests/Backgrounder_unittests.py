...
class Backgrounder_tests(unittest.TestCase):
    '''Testing Info
    - Set up several base functions that will delay
    -- Generator
    -- Closure (Getting Data from DB)
    -- Regular processing function (with args, and kwargs)
    
    Each test should:
    -- Set testing parameters (like the range of the data)
    -- Run the functions in the regular way
    -- Run them with backgrounder
    -- Validate Output
    -- Display Elapsed time for processing
    '''
    #Regular functions
    ...
    def test_regular_function(self):
        ...
        test2results = []
        a = time()
        bg=Backgrounder(self.slow_data_getting_function,fn_args=[1,test2],sentinel='sentinel')
        while bg.status():
            x = bg.get_one()
            if x and x != 'sentinel':
    ...
    def test_with_generator_as_input(self):
        ...
        a = time()
        test2results = []
        bg = Backgrounder(self.slow_generator,fn_args=[1,test2],generator=True)
        while bg.status():
            data = bg.get_one()
            if data:
                test2results.append(self.slow_processing_function(data))
    ...
    def test_nested_backgrounders(self):
        ...
        test2results = []
        a = time()
        bg_get = Backgrounder(self.slow_data_getting_function,fn_args=[1,test2],sentinel='sentinel_get',name='bg_get')
        bg_proc = Backgrounder(self.slow_processing_function,in_bg=bg_get,max_threads=1)
